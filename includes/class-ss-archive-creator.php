...
class Archive_Creator {
  ...
  public function fetch_pages() {
    ...
    while ( $static_page = array_shift( $static_pages ) ) {
      ...
      // Ignore wp-login, wp-admin
      if (strpos($static_page->url, 'wp-admin') !== false || strpos($static_page->url, 'wp-login') !== false) {
        continue;
      }
  ...
  public function publish_to_s3( $bucket, $aws_access_key_id, $aws_secret_access_key ) {
    ...
    foreach ( $recursive_iterator as $item ) {
      if (!$item->isDir()) {
        ...
        // skip wp-login or wp-admin links.
        if (strpos($path, 'wp-login') !== false || strpos($path, 'wp-admin') !== false) {
          continue;
        }
        try {
          $s3->putObject( \S3::inputFile( $item->getRealPath() ), $bucket, $path, \S3::ACL_PUBLIC_READ );
          if(strpos($item->getRealPath(), '404.html') !== false) {
            $s3->putObject( \S3::inputFile( $item->getRealPath() ), $bucket, '404.html', \S3::ACL_PUBLIC_READ );
          }
        } catch (any $err) {
